...
>
  <div class="container-x">
    <div class="flex h-16 items-center justify-between gap-4 sm:h-20">
      <!-- Logo -->
      <a href="#inicio" class="group flex items-center gap-2.5 text-sm font-semibold tracking-tight" aria-label="{{ __('header.logoAria') }}">
        <span
      ...
      <nav class="hidden items-center gap-8 lg:flex" aria-label="{{ __('nav.mainAria') }}">
        ...
        <a href="#projetos" class="nav-link">{{ __('nav.projects') }}</a>
        <a href="#stack" class="nav-link">{{ __('nav.stack') }}</a>
      ...
      <div class="flex items-center gap-2 sm:gap-3">
        ...
        <div class="relative" id="lang-switch">
          <button type="button" id="lang-toggle" class="flex h-10 items-center gap-1.5 rounded-full border px-3 text-xs font-semibold uppercase tracking-wider text-muted transition-colors duration-300 hover:border-line-strong hover:text-fg" aria-haspopup="true" aria-expanded="false" aria-controls="lang-menu" aria-label="{{ __('lang.aria') }}" data-cursor="hover">
            <span data-lang-current>{{ strtoupper($currentLocale) }}</span>
            <svg
          ...
          >
            @foreach ($localeLabels as $code => $label)
              ...
              >
                <span class="lang-code">{{ strtoupper($code) }}</span> {{ $label }}
              </a>
            @endforeach
          </div>
        </div>
  ...
  >
    <nav class="container-x flex flex-col gap-1 py-6" aria-label="{{ __('nav.mobileAria') }}">
      <a href="#sobre" class="rounded-lg px-3 py-3 text-lg font-medium text-muted transition-colors hover:bg-surface hover:text-fg">{{ __('nav.about') }}</a>
      <a href="#servicos" class="rounded-lg px-3 py-3 text-lg font-medium text-muted transition-colors hover:bg-surface hover:text-fg">{{ __('nav.services') }}</a>
      <a href="#projetos" class="rounded-lg px-3 py-3 text-lg font-medium text-muted transition-colors hover:bg-surface hover:text-fg">{{ __('nav.projects') }}</a>
      <a href="#stack" class="rounded-lg px-3 py-3 text-lg font-medium text-muted transition-colors hover:bg-surface hover:text-fg">{{ __('nav.stack') }}</a>
      <a href="#parceria" class="rounded-lg px-3 py-3 text-lg font-medium text-muted transition-colors hover:bg-surface hover:text-fg">{{ __('nav.partnership') }}</a>
      <a href="#contacto" class="btn btn-primary mt-4 w-full">{{ __('nav.cta') }}</a>
    </nav>
  </div>
</header>
